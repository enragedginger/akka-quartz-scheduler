...
[![Build Status](https://api.travis-ci.org/enragedginger/pekko-quartz-scheduler.svg?branch=master)](https://travis-ci.org/enragedginger/pekko-quartz-scheduler)

Quartz Extension and utilities for true scheduling in Pekko 1.0.x.

Current release is built for Scala 2.13.x and Pekko 1.0.x and is available on Maven Central. If you would like support
for a different combination of Scala and Pekko, simply post your request on the issues page (as well as a reason as to
why the currently available versions won't work for you. I'm always curious about these things).

# Why Pekko and Quartz?
...
to Pekko than the one already available.

The goal here is to provide Pekko with a scheduling system that is closer to what one would expect for Cron type jobs –
...
1. What's wrong with Pekko's existing Scheduler?
    As Viktor Klang points out, 'Perhaps the name "Scheduler" was unfortunate,
    "Deferer" is probably more indicative of what it does.'

    The Pekko Scheduler is designed to setup events that happen based on durations from the current moment:
    You can say "fire this job in 15 minutes, every 30 minutes thereafter" but not "fire a job every day at 3pm".

    Furthermore, Pekko's default scheduler is executed around a [`HashedWheelTimer`](http://docs.jboss.org/netty/3.1/api/org/jboss/netty/util/HashedWheelTimer.html) –
...
externalised to the Pekko configuration file; when a schedule request is made its name is matched up with an entry
...
```scala
// For Pekko 1.0.x and Pekko Typed Actors 1.0.x and Scala 2.12.x, 2.13.x, 3.1.x
libraryDependencies += "com.enragedginger" %% "pekko-quartz-scheduler" % "1.9.3-pekko-1.0.x"

```

Note that the version name includes the Pekko revision (Previous releases included the Pekko release in the artifact name, which broken Maven).

### Pekko Typed Actor
From within your Pekko project you can create and access a Typed Scheduler:

```scala
// Pekko Typed Actors sample.
...
Where `typedSystem` represents an instance of an Pekko Typed `ActorSystem[-T]` – note that `QuartzSchedulerTypedExtension` 
...
Note that you can use the same logic for scheduling Quartz with Pekko Typed Actor as the older version.

### Pekko Classic Actor
From within your Pekko project you can create and access a Scheduler:
...
Where `system` represents an instance of a Pekko `ActorSystem` – note that `QuartzSchedulerExtension` is scoped
...
messages were dealt with more immediately than "normal" actor messages, they could utilize [Priority Mailboxes](https://pekko.apache.org/docs/pekko/current/dispatchers.html?language=scala).
...
All configuration of `pekko-quartz-scheduler` is done inside of the Pekko configuration file in an `pekko.quartz` config
block. Like Pekko's configuration file, this follows the [HOCON Configuration Format](https://github.com/typesafehub/config/blob/master/HOCON.md).
